...
namespace Integradora.Norma.Winthor.Magento002
{
    public class Comum
    {
        static internal void MarcarPosicaoPedido(Integradora.ECommerce.Magento002.Integracao LojaECommerce
                                        ...
                                        , string email)
        {
            ...
            {
                Logs.GerarLog("Integradora.Norma.Winthor.Magento002.Comum"
                                                  , "MarcarPosicaoPedido"
        ...
        static internal bool ExecutarIntegradora(decimal numPedido, decimal codRCA, Integradora.ERP.Winthor.Domain.PedidoWeb PedidoForcaVenda)
        {
            ...
            {
                Logs.GerarLog("Integradora.Norma.Winthor.Magento002.Comum", "ExecutarIntegradora", "Executando integradora do Winthor no pedido: " + numPedido.ToString());
            }
            ...
            using (var transaction = new System.Transactions.CommittableTransaction())
            {
                try
                {
                    ...
                    if (Biblioteca.Dictionaries.Settings.Debugando())
                    {
                        Logs.GerarLog("Integradora.Norma.Winthor.Magento002.Comum", "ExecutarIntegradora", "Concluido a integradora do Winthor no pedido: " + numPedido.ToString());
                    }
            ...
            if (importado)
            {
                PCPedctemp pedidogerou = PCPedctemp.Listar("NUMPEDRCA = " + numPedido.ToString()).FirstOrDefault();
                importado = pedidogerou.Importado == "S";
                if (!importado)
                {
                    Logs.GerarLog("Integradora.Norma.Winthor.Magento002.Comum"
                                                      , "ExecutarIntegradora"
                                                      , "Pedido: " + numPedido.ToString() + " erro: " + pedidogerou.Observacao_Pc);
                }
            }
            return importado;
        }
