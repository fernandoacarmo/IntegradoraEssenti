                        ...
                        foreach (DataColumn column in drData.Table.Columns)
                        {
                            if (((XmlElementAttribute)aTargetAttribute).ElementName.ToUpper() == column.ColumnName.ToUpper())
                            {
                                if (drData[column.ToString()] != DBNull.Value)
                                {
                                    ...
                                    if ((tColumnDataType != null) && (tColumnDataType == typeof(System.String)) && 
...
//                                        var convertedValue = Convert.ChangeType(drData[column.ToString()], piTargetProperty.PropertyType);
                                        //piTargetProperty.SetValue(targetClass, convertedValue, null);
